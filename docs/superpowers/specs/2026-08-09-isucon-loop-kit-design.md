...
- チーム人数: 1 人
- 本番主言語: Python
- 準備方針: ツール寄り（過去問はドックフーディング検証用に最小限）
...
| `bench-note` | ベンチ後にユーザーがスコアを報告し履歴に追記（前回/最高と比較、低下時は確認） |
...
5. ベンチ
6. `pull` && `analyze`
7. `bench-note` でスコアを報告 → ↑なら残す / ↓や破壊なら `rollback`
...
| ツール作り込みすぎて当日手順が未練度 | ドックフーディングは必須。スコア練習は後回しでよいがフロー通しは必須 |
